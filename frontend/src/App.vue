...
onMounted(async () => {
  ...
  if (store.targets.length === 0) {
    hosts.forEach((h: any) => {
      // Verifique se o backend está enviando como 'isGateway' (conforme seu entity.go)
      store.addTarget(
        h.id,
        h.ip,
        h.name,
...
<template>
  <div
    class="flex flex-col md:flex-row h-screen bg-[#050505] text-gray-300 font-sans selection:bg-green-500/30 selection:text-green-200 overflow-hidden"
  >
    <aside
      class="w-full md:w-64 bg-black border-b md:border-b-0 md:border-r border-gray-900 flex flex-row md:flex-col justify-between shrink-0 z-50"
    >
      <div
        class="flex flex-row md:flex-col items-center md:items-stretch w-full"
      >
        <div
          class="h-16 flex items-center justify-center border-b-0 md:border-b border-gray-900 px-6 shrink-0"
        >
        ...
        <nav
          class="flex flex-row md:flex-col flex-1 md:mt-8 space-y-0 md:space-y-2 px-2 gap-1 md:gap-0 overflow-x-auto md:overflow-x-visible no-scrollbar"
        >
          <button
